...
class ReportDataUpdaterService {
    ...
    def updateReports (){
        ...
        parseINC()
        actCSV = new File("data/lsps_activities.csv")
        ...
        parseACT()
        prbCSV = new File("data/lsps_problems.csv")
    ...
    // This function parses list of owner names and EIDs
    def parseOwners() {
...
//    Returns a list of worklogs
    private List<ODWorklog> parseWorklog(List<CSVRecord> records, String worklogTitle) {
        ...
        while (isValidRecord(record)) {
            ...
            Date date = new Date().parse(FORMAT2, record.get(worklogColumnIndexes.get(WORKLOG_DATE)));
