...
<script>
    function dashboard(id, fData){
        ...
        function segColor(c){ return { worklog:"#41ab5d",noWorklog:"#ff0000"}[c]; }

        // compute total for each request type.
        fData.forEach(function(d){d.total=d.freq.worklog+d.freq.noWorklog;});

        // function to handle histogram.
        function histoGram(fD, color){
...
//                    .call(d3.svg.axis().scale(x).orient("bottom"));
            ...
            function mouseover(d){  // utility function to be called on mouseover.
                // filter for selected state.
                    ...
                    tip = d3.tip()
                            ...
                            .offset([-10, 0])
                            .html(function (d) {
                                return "<strong>Request Type:</strong> <span style='color:red'>" + st.State + "</span>";
                            })

                    hGsvg.call(tip)
                    tip.show()
                    // call update functions of pie-chart and legend.
                if(nD.filter(function(d) { return d.freq > 0 }).length > 0) {
                    pC.update(nD);
                }
                    leg.update(nD);
            ...
            function mouseout(d){    // utility function to be called on mouseout.
                ...
                pC.update(tF);
                leg.update(tF);
                hGsvg.on('mouseOut', tip.hide())
            }

            // create function to update the bars. This will be used by pie-chart.
            hG.update = function(nD){
...
//                            .attr("transform", "translate("+pieDim.w/2+","+pieDim.h/2+")").append("svg");
        ...
        // function to handle pie chart legend.
        function pieChartLegend(lD){
            var leg = {};
...
//                        chart.toggle(id);
        ...
        barGraphLegend(requestToColor, color, hG)
    }
</script>
